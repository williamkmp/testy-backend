...
public class BlockSeeder implements Seeder {
    ...
    public void seed() throws Exception {
        Page testyRootPage = pageService
            .findById(1L)
            .orElseThrow(() -> new Exception("Page with id [1] is not found"));
        Page bimayRootPage = pageService
            .findById(2L)
            .orElseThrow(() -> new Exception("Page with id [2] is not found"));
        Page bimobRootPage = pageService
            .findById(3L)
            .orElseThrow(() -> new Exception("Page with id [3] is not found"));
    ...
    private List<Block> generateBlockList() {
        ...
        blocks.add(
            new Block()
                ...
                .setContent(
                    "<p>Please Stop Using Autowired Field Injection in the Spring</p>"
                )
        );
        ...
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.HEADING_2)
                .setContent("<p>Dependency injection types</p>")
        );
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.PARAGRAPH)
                .setContent(
                    "<p>There are three dependency injection ways in spring.</p>"
                )
        );
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.BULLET_LIST)
                .setContent("<p>Constructor injection</p>")
        );
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.BULLET_LIST)
                .setContent("<p>Setter Injection</p>")
        );
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.BULLET_LIST)
                .setContent("<p>Field Injection</p>")
        );
        blocks.add(
            new Block()
                .setBlockType(BLOCK_TYPE.HEADING_3)
                .setContent("<p>Constructor injection</p>")
        );
        blocks.add(new Block().setBlockType(BLOCK_TYPE.DIVIDER));
        blocks.add(
